...
func verifySignature(x5u, token string, verifyCA bool) (string, int, error) {
	...
	if err != nil {
		logError("%v", err)
		return "VESPER-4200", http.StatusBadRequest, err
	}
	...
	if err != nil {
		logError("%v", err)
		return "VESPER-4201", http.StatusBadRequest, err
	}
	block, _ := pem.Decode(cert_buffer)
	if block == nil {
		err = fmt.Errorf("no PEM data is found")
		return "VESPER-4202", http.StatusBadRequest, err
	}
	// parse certificate
	cert, err := x509.ParseCertificate(block.Bytes)
	if err != nil {
		return "VESPER-4203", http.StatusBadRequest, err
	}
	now := time.Now()
	...
	if _, err := cert.Verify(opts); err != nil {
		...
		case "x509: certificate has expired or is not yet valid":
			return "VESPER-4204", http.StatusBadRequest, err
		case "x509: certificate signed by unknown authority" :
			if verifyCA {
				return "VESPER-4205", http.StatusBadRequest, err
			}
		case "x509: certificate is not authorized to sign other certificates":
			if verifyCA {
				return "VESPER-4206", http.StatusBadRequest, err
			}
		case "x509: issuer name does not match subject from issuing certificate":
			if verifyCA {
				return "VESPER-4207", http.StatusBadRequest, err
			}
		default:
			if verifyCA {
				return "VESPER-4208", http.StatusBadRequest, err
			}
		}
	}
	...
	if !ok {
		...
		return "VESPER-4209", http.StatusBadRequest, err
	}
	err = verifyEC(token, ecdsa_pub)
	if err != nil {
		return "VESPER-4210", http.StatusUnauthorized, err
	}
