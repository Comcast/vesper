...
func signRequest(response http.ResponseWriter, request *http.Request, _ httprouter.Params) {
	...
	case err == io.EOF:
		...
		serveHttpResponse(start, response, lg, http.StatusBadRequest, "error", traceID, "VESPER-4001", "empty request body")
		return
	case err != nil :
		lg := kitlog.With(glogger, "type", "requestPayload", "clientIP", clientIP, "module", "signRequest")
		serveHttpResponse(start, response, lg, http.StatusBadRequest, "error", traceID, "VESPER-4002", "unable to parse request body")
		return
	default:
		// err == nil. continue
	}
	...
	if err != nil {
		lg := kitlog.With(glogger, "type", "requestPayload", "clientIP", clientIP, "module", "signRequest")
		serveHttpResponse(start, response, lg, http.StatusBadRequest, "error", traceID, errCode, err.Error())
		return
	}
	...
	if err != nil {
		lg := kitlog.With(glogger, "type", "requestPayload", "clientIP", clientIP, "module", "signRequest")
		serveHttpResponse(start, response, lg, http.StatusInternalServerError, "error", traceID, "VESPER-5050", fmt.Sprintf("%v - error in converting header to byte array", err))
		return
	}
	claimsBytes, _ := json.Marshal(orderedMap)
	if err != nil {
		lg := kitlog.With(glogger, "type", "requestPayload", "clientIP", clientIP, "module", "signRequest")
		serveHttpResponse(start, response, lg, http.StatusInternalServerError, "error", traceID, "VESPER-5051", fmt.Sprintf("%v - error in converting claims to byte array", err))
		return
	}
	canonicalString, sig, err := createSignature(hdrBytes, claimsBytes, []byte(p))
	if err != nil {
		lg := kitlog.With(glogger, "type", "requestPayload", "clientIP", clientIP, "module", "signRequest")
		serveHttpResponse(start, response, lg, http.StatusInternalServerError, "error", traceID, "VESPER-5052", fmt.Sprintf("%v - error in signing request for request payload", err))
		return
	}
	lg := kitlog.With(glogger, "type", "requestResponseTime", "module", "signRequest")
